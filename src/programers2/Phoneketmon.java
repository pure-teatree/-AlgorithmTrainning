...
public class Phoneketmon {
	public static int solution(int[] nums) {
        ...
        Arrays.sort(nums);//정렬
        int temp=nums[0];//첫번째 값
        for(int i =1;i<nums.length;i++) {
        	if(temp==nums[i]) { //저장해놓은 값과 비교
        		answer --; //같은 포켓몬이면 뺌
        	}else {
        		temp=nums[i]; //다음 값과 비교하기위해
        	}
        }
        if(answer >=nums.length/2) {//선택할 수 있는 최대 수보다 포캣몬이 많다면
        	answer = nums.length/2; //선택할수있는 최대수로 선택 가능
        }
